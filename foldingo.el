;;; $Id: foldingo.el,v 1.37 2001/12/24 07:17:26 queinnec Exp $
...
;;; Keywords: folding mode
;;; Created: 11 november 1995
;;; Description: A minor folding mode with mouse/menu/face support.
;;; URL: http://www-spi.lip6.fr/~queinnec/WWW/elisp.html
;;; Compatibility: Emacs19, Emacs20
;;; Last-Updated: $Date: 2001/12/24 07:17:26 $

;;; LCD Archive Entry:
;;; fold|Christian Queinnec|Christian.Queinnec@lip6.fr|
;;; A minor folding mode with mouse/menu/face support.|
;;; $Date: 2001/12/24 07:17:26 $|$Revision: 1.37 $|
;;; ~/misc/foldingo|
...
;;;   http://www-spi.lip6.fr/~queinnec/Miscellaneous/foldingo-1.24.el
...
;;;{{{ Remarks
;;; This code was tested with Emacs 19.29. Currently, it still works
;;; with Emacs 20.7. It also works on read-only buffers as well as on
;;; Lisp, Scheme, C, Caml files.
...
;;; it while folded), and just add the following line to your .emacs:
;;;       (require 'foldingo)
;;;
              ...
              (let ((n (length fold-folds-list)))
                (if (= 0 n)
                    (progn (message "Found 0 folds: Fold-mode exited")
                           (fold-mode nil) )
                  (message "Looking for folds... (%d found) - Done." n)
		  ) ) )
          (progn
            (setq 
...
"
  (interactive)
  ;; Discovered by <michael.brand@siemens.com>, return if these three
  ;; variables are defined. This avoid requiring comment-start to be
  ;; defined (it is not in the text-mode for instance).
  (if (not (and fold-end-regexp
                fold-begin-regexp
                fold-end-or-begin-regexp ))
      (let (cs)
        ;; Discovered by Nadim Shaikli <shaikli@yahoo.com>
...
;;; Michael Brand <michael.brand@siemens.com> defines this for text-mode:
;;;(put 'text-mode       'fold-end-regexp          "^[ \t]*}}}")
;;;(put 'text-mode       'fold-begin-regexp        "^[ \t]*{{{")
;;;(put 'text-mode       'fold-end-or-begin-regexp "^[ \t]*\\(}}}[
;;;\t]*\\|{{{\\)")
;;;   }}}
...
;;                   (/ (* 100 dot) (point-max)) )
          (if (looking-at fold-end-regexp)
              (if (consp stack-begin)
                ...
                (error "Unexpected %s at line %d" 
                       fold-end (+ 1 (count-lines 1 dot))) ) )
          ...
          (end-of-line) ) ) )
    (if (consp stack-begin)
        (error "Unmatched %s at line %d" 
               fold-begin (+ 1 (count-lines 1 (car stack-begin))) ) ) )
...
;;; Insert a new fold. After Georg Greve's code suggested by Nadim
;;; Shaikli <shaikli@yahoo.com>. One may prefer (as in lisp) to use
;;; something like ";;; " rather than a plain ";".

(defun insert-new-empty-fold (foldname)
  "Insert a new empty fold.
The name of the fold may be prompted for.
Sets the dot within the fold."
  (interactive "*sTitle of the new fold: ")
  (insert-opening-fold foldname)
  (let ((old (point)))
    (insert-closing-fold)
    (goto-char old) ) )

(defun insert-opening-fold (foldname)
  "Open a new empty fold.
The name of the fold may be prompted for."
  (interactive "*sTitle of the new fold: ")
  (if (not (bolp))
      (insert ?\n))
...
;;;   }}}{{{ ADD ON by Georg Greve (for C++)
;;; The following functions are not exactly equivalent to the previous set
;;; therefore I left them not to bother old users.
...
;;; Add those bindings to the fold-keymap
;(define-key fold-keymap "\C-cfi"   'insert-new-c++-fold)
;(define-key fold-keymap "\C-cfc"   'close-c++-fold)
(define-key fold-keymap "\C-cfi"   'insert-opening-fold)
(define-key fold-keymap "\C-cfw"   'insert-new-empty-fold)
(define-key fold-keymap "\C-cfc"   'insert-closing-fold)
(define-key fold-keymap "\C-cfe"   'fold-enter-mode) 
(define-key fold-keymap "\C-cff"   'fold-close-all-folds) 
(define-key fold-keymap "\C-cfu"   'fold-open-all-folds) 
(define-key fold-keymap "\C-cfo"   'fold-open-current-fold) 
(define-key fold-keymap "\C-cfh"   'fold-close-current-fold) 
(define-key fold-keymap "\C-cfa"   'fold-enter-fold-mode-close-all-folds) 
	...
	(while overlays
	  (let ((o (car overlays)))
		...
		(if (< (overlay-start o) beg)
			...
			(let ((o1 (foldingo-copy-overlay o)))
...
(defun foldingo-copy-overlay (o)
  "Copy an overlay preserving its properties."
  (let ((newo (make-overlay (overlay-start o) (overlay-end o))))
    (mapcar (lambda (p)
              (foldin-overlay-put newo p (overlay-get o p)) )
            '(face invisible foldingo ) )
    newo ) )
...
(if (check-for-XEmacs)
    (progn
	  ...
	  "----"
	  ["Insert new empty fold" insert-empty-fold                t]
          ["Open fold"             insert-opening-fold              t]
	  ["Close fold"            insert-closing-fold              t]
          "----"
...
;;; Don't forget to define them in reverse order.
    ...
    (define-key fold-menu-map [fold-separator1]
      '("--") )
    (define-key fold-menu-map [insert-closing-fold]
      '("Close fold" . insert-closing-fold) )
    (define-key fold-menu-map [insert-new-empty-fold]
      '("Insert fold" . insert-new-empty-fold) )
    (define-key fold-menu-map [insert-opening-fold]
      '("Open fold" . insert-opening-fold) )
    (define-key fold-menu-map [fold-separator1]
      '("--") )
